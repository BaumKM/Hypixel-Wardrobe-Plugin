...
public class OpenWardrobe implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(sender instanceof Player p){
            if(!Main.getInstance().isSQLConnected){ p.sendMessage(ChatColor.RED + "This feature is currently disabled"); return true;}
            if(args.length == 0){
                ...
                if(p.hasPermission("wardrobe.clear.confirm")){
                    ...
                    if(now.isBefore(instant)){
                        try {
                            ...
                            WardrobeGUI playerWardrobe1 = WardrobeMap.getPlayer(Main.getInstance().getServer().getPlayer(target),1);
                            WardrobeGUI playerWardrobe2 = WardrobeMap.getPlayer(Main.getInstance().getServer().getPlayer(target),2);
                            playerWardrobe1.resetPlayerArray();
